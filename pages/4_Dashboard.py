...
if "Asset_ID" in equipment_df.columns:
    equipment_df["Asset_ID"] = equipment_df["Asset_ID"].astype(str).str.strip()
...
# Merge Maintenance Date into Equipment
...
# --- Layout Toggles (preserving your existing logic) ---
...
if os.path.exists(layout_file):
    ...
    st.session_state.visible_widgets = {
        "kpis": True, "status_chart": True, "inventory_table": True,
...
# Location Filter (NEW)
location_col = next((col for col in equipment_df.columns if col.lower() == "location"), None)
if location_col:
    locations = equipment_df[location_col].dropna().unique().tolist()
    location_filter = st.sidebar.selectbox("📍 Filter by Location", ["All"] + locations)
    if location_filter != "All":
        equipment_df = equipment_df[equipment_df[location_col] == location_filter]

# Maintenance Status Filter (NEW)
if "maintenance_status" in equipment_df.columns:
    status_options = equipment_df["maintenance_status"].unique().tolist()
    status_filter = st.sidebar.selectbox("🛠 Maintenance Filter", ["All"] + status_options)
    if status_filter != "All":
        equipment_df = equipment_df[equipment_df["maintenance_status"] == status_filter]
